...
public class PanchayatWise extends Action
{
	String sqlQry="";
	public String process(HttpServletRequest request, Bean bean)
	{
		...
		String panchayat = serviceBean.getServiceName();
		Statement stmt = null;
		ResultSet rs = null;
		...
		try 		
		{
			...
			sqlQry =  "Select distinct to_char(PersonMaster.datetime,'dd/MM/yyyy')dt,firstName,LastName,Gender,Age," +
					"FatherName,Contact, VillageName  from PersonMaster " +
					...
					"PersonChild.ServiceId = PersonChild.ServiceId where PersonMaster.Panchayatid = '" + panchayat + "' ";
			...
			ArrayList panchayatList = new ArrayList();
				...
				while(rs.next())
				{
					...
					panchayatList.add(map);				
				}
				request.setAttribute("path", "Report/panchayatWise.jsp");
				request.setAttribute("panchayat", panchayatList);								
	...
	public void criteria(String from, String to)
	{
		...
		sqlQry +=  " order by VillageName asc";
	}
}
